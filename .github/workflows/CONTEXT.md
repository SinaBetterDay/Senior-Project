...
- **`deploy` job** — Railway (`railway up`), needs `test`, runs **only** on push to `main`. Requires repository secrets `RAILWAY_TOKEN` (project token) and `RAILWAY_SERVICE_ID` (service name or ID).
